...
static const char *RawDirName = "/sdcard/sunny/raw/";
...
void OnBufferCallback(void *ctx, AImageReader *reader, AHardwareBuffer* buffer) {
  reinterpret_cast<ImageReader *>(ctx)->BufferCallback(reader, buffer);
}

/**
 * Constructor
 */
ImageReader::ImageReader(ImageFormat *res, enum AIMAGE_FORMATS format, int camId,int S)
    : reader_(nullptr),s(0), presentRotation_(0){
  callback_ = nullptr;
  ...
  AImageReader_BufferRemovedListener bufRmListener{
      .context = this, .onBufferRemoved = OnBufferCallback,
  };
  AImageReader_setBufferRemovedListener(reader_, &bufRmListener);
}

ImageReader::~ImageReader() {
  LOGI("~ImageReader() for Cam %d ==1\n", camIdImage);
  ASSERT(reader_, "NULL Pointer to %s", __FUNCTION__);
  AImageReader_delete(reader_);
  LOGI("~ImageReader() for Cam %d ==2\n", camIdImage);
}
...
void ImageReader::ImageCallback(AImageReader *reader) {
  ...
  if (format == AIMAGE_FORMAT_YUV_420_888) {
    ...
    AImage_getTimestamp(image,&clockCurTime);
    if (clockFirstTime == 0)
        clockFirstTime = clockCurTime;
    diffTime = clockCurTime - clockFirstTime;
    clockFirstTime = clockCurTime;
    if (camIdImage == 0) {
        printf("RgbImageCallback diffTime = %lldns, for camId=%d\n", (long long)diffTime, camIdImage);
		if(s) {
    ...
    if (camIdImage == 1) {
        printf("FishEyeImageCallback diffTime = %lldns, for camId=%d\n", (long long)diffTime, camIdImage);
  ...
  } else if (format == AIMAGE_FORMAT_RAW_PRIVATE) {
      media_status_t status = AImageReader_acquireNextImage(reader, &image);
      ASSERT(status == AMEDIA_OK && image, "Image is not available");
      AImage_getTimestamp(image,&clockCurTime);
      if (clockFirstTime == 0)
      ...
      diffTime = clockCurTime - clockFirstTime;
      clockFirstTime = clockCurTime;
      printf("TofImageCallback diffTime = %lldns, for camId=%d\n", (long long)diffTime, camIdImage);

      // Create a thread and write out the jpeg files
      if(s) {
        std::thread writeRawFile(&ImageReader::saveTofRawData, this, image);
        writeRawFile.detach();
     } else {
        if(image) AImage_delete(image);
     }
...
void ImageReader::BufferCallback(AImageReader *reader, AHardwareBuffer* buffer) {
    printf("Cam%d, BufferCallback buffer%p removed\n", camIdImage, buffer);
}
...
void ImageReader::WriteFile(AImage* image) {
 ...
 // int planeCount;
  ...
  struct timespec ts {
    ...
    /*******when use -r or -R, set fishWight:2560  fishHight:800    ****/
    ...
    fileName = kDirName;
    fileName += "fish_eye"+to_string(i)+".raw";
...
void ImageReader::saveTofRawData(AImage* image)
{
   // char file_name[64] = {0};
    DIR *dir;
    int planeCount;
    int len=0;
    static int rawImgNum=1;
    uint8_t *data = nullptr;
    media_status_t status = AImage_getNumberOfPlanes(image, &planeCount);
    ASSERT(status == AMEDIA_OK,
          "Error: getNumberOfPlanes() planeCount = %d", planeCount);
    AImage_getPlaneData(image, 0, &data, &len);
    dir = opendir(RawDirName);
    if (dir) {
      closedir(dir);
    } else {
      std::string cmd = "mkdir -p ";
    ...
    string fileName = RawDirName;
    fileName +="tof_original_"+ to_string(rawImgNum)+".raw";
  //  sprintf(file_name, "/sdcard/sunny/raw/tof_original_%06d.raw", rawImgNum++);
