...
void OnSessionClosed(void* ctx, ACameraCaptureSession* ses) {
  ACameraDevice* dev;
  ACameraCaptureSession_getDevice(ses, &dev);
  if (dev != NULL) {
      std::string id(ACameraDevice_getId(dev));
      LOGW("session %p closed for %s", ses, id.c_str());
  } else {
      LOGW("session %p closed and device already closed", ses);
  }
...
void OnSessionReady(void* ctx, ACameraCaptureSession* ses) {
  ...
  if (dev != NULL) {
      std::string id(ACameraDevice_getId(dev));
      LOGW("session %p ready for %s", ses, id.c_str());
  } else {
      LOGE("ready never reach here");
  }
  reinterpret_cast<NDKCamera*>(ctx)
...
void OnSessionActive(void* ctx, ACameraCaptureSession* ses) {
  ...
  if (dev != NULL) {
      std::string id(ACameraDevice_getId(dev));
      LOGW("session %p active for %s", ses, id.c_str());
  } else {
      LOGE("active never reach here");
  }
...
void NDKCamera::OnSessionState(ACameraCaptureSession* ses,
  ...
  LOGW("OnSessionState ses=%p captureSession_=%p", ses, captureSession_);
  // TODO: delete compare with captureSession_ , ses is the real value.
  if (ses != NULL) {
    ACameraDevice* dev;
    ACameraCaptureSession_getDevice(ses, &dev);
    if (dev != NULL) {
        std::string id(ACameraDevice_getId(dev));
        LOGW("ses=%p state=%d for %s", ses, state, id.c_str());
    }
    if (ses != captureSession_) {
        LOGW("CaptureSession is NOT our session");
        return;
    }
  } else {
    LOGE("ses is NULL, return");
    return;
  }
